export class Navigation {
    constructor(options) {
        ...
        new ResizeObserver(() => {
            for (const dropdown of this.dropdowns) {
                this.#isDesktop() ? this.#registerDropdownEvents(dropdown) : this.#unregisterDropdownEvents(dropdown);
            }
     ...
     */
    #merge(a, b) {
        // biome-ignore-start lint/performance/noAccumulatingSpread: reason
        ...
        // biome-ignore-end lint/performance/noAccumulatingSpread: reason
    }
    ...
    #initFocusTrapTargets() {
        const nodes = [
            this.navigation.closest('#header')?.querySelector('a[href].logo'),
    ...
    #init() {
        ...
        document.addEventListener('keyup', (e) => {
            e.key === 'Escape' && this.#hideDropdown();
        });
    }
    ...
    #updateAriaState(dropdown, show) {
        dropdown.btn.ariaLabel =
            (show ? this.options.ariaLabels.collapse : this.options.ariaLabels.expand) + dropdown.btn.dataset.label;
    ...
    #collapseSubmenu(dropdown) {
        ...
        dropdown
            ...
            ?.classList.remove(this.options.classes.boundsLeft, this.options.classes.boundsRight);
